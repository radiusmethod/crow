...
| tmux backend not starting                                | The tmux backend is the default since #301. If tmux is missing or < 3.3, Crow surfaces a launch alert with a `brew install tmux` hint and falls back to Ghostty for that launch. Verify with `tmux -V`. Set `CROW_TMUX_BACKEND=0` to opt out of tmux entirely for a launch (escape hatch — will be removed in a follow-up release). The flag is frozen at launch — relaunch after changing the env var. |

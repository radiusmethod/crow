...
`--corveil-auto-update` is live and **on by default** (CROW-1229): Crow checks at daemon start and on the existing `crow version` interval, checksum-verifies the public `corveil/corveil-releases` asset for this host, hot-swaps `.claude/bin/corveil`, and reinstalls skills. A missing config key decodes as on; `crow defaults set --corveil-auto-update false` stays off. A `binaries["corveil"]` path that is **not** under `~/Library/Application Support/crow/bin/corveil/` is treated as an operator source build and is never overwritten. Offline / checksum-mismatch / missing-asset keep the last-good binary and never break startup. The `-releases` mirror can trail source by a day.

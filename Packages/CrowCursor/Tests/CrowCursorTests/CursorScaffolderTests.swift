...
@Suite("CursorScaffolder")
struct CursorScaffolderTests {
    private func makeTempDevRoot() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("cursor-scaffold-\(UUID().uuidString)")
    ...
    @Test func scaffoldWritesAgentsMD() throws {
        ...
        try CursorScaffolder.scaffold(devRoot: devRoot.path)
    ...
    @Test func scaffoldPreservesUserCorrections() throws {
        ...
        let edited = template.replacingOccurrences(
            ...
            with: "## Known Issues / Corrections\n\n- Use `crow new-session --agent cursor` for Cursor sessions."
        )
        ...
        #expect(after.contains("Use `crow new-session --agent cursor`"))
    }
}

...
@Suite("CursorSignalSource")
struct CursorSignalSourceTests {
    private let source = CursorSignalSource()
    ...
    // MARK: - Canonical event vocabulary (PascalCase via writer rewrite)
    ...
    // MARK: - Notification (afterAgentResponse safety net)

    @Test func notificationMarksDoneWhenNoStopYet() {
        let t = source.transition(
        ...
        #expect(t.newActivityState == .done)
        if case .clear = t.toolActivity {} else {
            Issue.record("Notification should clear tool activity")
        }
    }

    @Test func notificationDoesNotOverrideExistingStop() {
        let stopAt = Date()
        let t = source.transition(
            for: event("Notification"),
            currentActivityState: .done,
            currentNotificationType: nil,
            currentLastTopLevelStopAt: stopAt
        )
        // Already stopped — Notification should not flip activity state.
        #expect(t.newActivityState == nil)
    }

    // MARK: - PermissionRequest (defensive parity)
